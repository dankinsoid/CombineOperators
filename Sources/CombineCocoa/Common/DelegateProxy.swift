...
//  CombineCocoa
//
//  Adapted from RxCocoa DelegateProxy
...
open class DelegateProxy<P: AnyObject, D>: _CBDelegateProxy {
    ...
    /**
     Returns publisher of invocations of delegate methods. Elements are sent *before method is invoked*.
     ...
     or not. Observing is performed by installing a hidden associated `PassthroughSubject` that is
     ...
     * there is no sensible automatic way to determine a default return value
     ...
     manually installing a `PassthroughSubject` or `CurrentValueSubject` and implementing delegate method.

     e.g.

         // delegate proxy part (ScrollViewDelegateProxy)

         let internalSubject = PassthroughSubject<CGPoint, Never>()

         public func requiredDelegateMethod(scrollView: UIScrollView, arg1: CGPoint) -> Bool {
             internalSubject.send(arg1)
     ...
     ....
         ...
         public var property: AnyPublisher<CGPoint, Never> {
             let proxy = ScrollViewDelegateProxy.proxy(for: base)
             return proxy.internalSubject.eraseToAnyPublisher()
         }
     ...
     **In case calling this method prints "Delegate proxy is already implementing `\(selector)`,
     ...
     - returns: Publisher of arguments passed to `selector` method.
     */
    open func sentMessage(_ selector: Selector) -> AnyPublisher<[Any], Never> {
        MainScheduler.ensureRunningOnMainThread()
    ...
    /**
     Returns publisher of invoked delegate methods. Elements are sent *after method is invoked*.
     ...
     **In case calling this method prints "Delegate proxy is already implementing `\(selector)`,
     ...
     - returns: Publisher of arguments passed to `selector` method.
     */
    open func methodInvoked(_ selector: Selector) -> AnyPublisher<[Any], Never> {
        MainScheduler.ensureRunningOnMainThread()
    ...
    fileprivate func checkSelectorIsObservable(_ selector: Selector) {
        ...
        if !(self._forwardToDelegate?.responds(to: selector) ?? true) {
            print("⚠️ Using delegate proxy dynamic interception method but the target delegate object doesn't respond to the requested selector. " +
                "In case pure Swift delegate proxy is being used please use manual observing method by using`PassthroughSubject`s. " +
...
private final class MessageDispatcher {
    ...
    private let result: AnyPublisher<[Any], Never>
    @Locked private var _subscriptionCount = 0
    ...
    init<P, D>(selector: Selector, delegateProxy _delegateProxy: DelegateProxy<P, D>) {
        ...
        self._subscriptionCount = locked.wrappedValue

        self.result = subject
            .handleEvents(
                receiveSubscription: { _ in
                    locked.wrappedValue += 1
                    if locked.wrappedValue == 1 {
    ...
    func asPublisher() -> AnyPublisher<[Any], Never> {
        return result
    }
}
