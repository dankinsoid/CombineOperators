...
//  CombineCocoa
//
//  Adapted from RxCocoa DelegateProxyType
...
/**
`DelegateProxyType` protocol enables using both normal delegates and Combine publishers with
...
This is more or less how it works.
      ...
      +-----------v-------------------------------+
      |                                           |
      | Delegate proxy : DelegateProxyType        +-----+---->  Publisher<T1>
...
Since CombineCocoa needs to automagically create those Proxies and because views that have delegates can be hierarchical
...
... this mechanism can be extended by using the following snippet in `registerKnownImplementations` or in some other
     part of your app that executes before using `cb.*` (e.g. appDidFinishLaunching).

    ScrollViewDelegateProxy.register { TableViewDelegateProxy(parentObject: $0) }
...
extension DelegateProxyType {
    ...
    /// When make 'CB*DelegateProxy' subclass, call 'CB*DelegateProxySubclass.register(for:_)' 1 time, or use it in DelegateProxyFactory
    /// 'CB*DelegateProxy' can have one subclass implementation per concrete ParentObject type.
    ...
    ///             let source = self.delegate.sentMessage(#selector(UISearchBarDelegate.searchBar(_:textDidChange:)))
    ///             ...
    ///         }
    ///     }
    ...
    public static func installForwardDelegate(_ forwardDelegate: Delegate, retainDelegate: Bool, onProxyForObject object: ParentObject) -> AnyCancellable {
...
extension Publisher where Failure == Never {
    func subscribeProxyDataSource<DelegateProxy: DelegateProxyType>(
        ...
        binding: @escaping (DelegateProxy, Output) -> Void
    ) -> AnyCancellable
...
/**
 ...
 part of your app that executes before using `cb.*` (e.g. appDidFinishLaunching).
     ...
     class ScrollViewDelegateProxy: DelegateProxy {
         public static func registerKnownImplementations() {
             self.register { TableViewDelegateProxy(parentObject: $0) }
     }
     ...
...
private class DelegateProxyFactory {
    ...
    fileprivate func extend<DelegateProxy: DelegateProxyType, ParentObject>(make: @escaping (ParentObject) -> DelegateProxy) {
            ...
            guard self._factories[ObjectIdentifier(ParentObject.self)] == nil else {
                cbFatalError("The factory of \(ParentObject.self) is duplicated. DelegateProxy is not allowed of duplicated base object type.")
            }
    ...
    fileprivate func createProxy(for object: AnyObject) -> AnyObject {
        ...
        cbFatalError("DelegateProxy has no factory of \(object). Implement DelegateProxy subclass for \(object) first.")
    }
}
