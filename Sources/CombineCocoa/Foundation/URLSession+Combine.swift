...
public extension Reactive where Base: URLSession {
	...
	/// ```
	func response(request: URLRequest) -> URLSession.DataTaskPublisher {
	...
	func data(request: URLRequest) -> AnyPublisher<Data, Error> {
		response(request: request).tryMap { pair -> Data in
	...
	func json(request: URLRequest, options: JSONSerialization.ReadingOptions = []) -> AnyPublisher<Any, Error> {
		data(request: request).tryMap { data -> Any in
			do {
	...
	func json(url: Foundation.URL) -> AnyPublisher<Any, Error> {
