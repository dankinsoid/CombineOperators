 ...
 */
extern BOOL CBAbortOnThreadingHazard;

/// Error domain for CBObjCRuntime.
extern NSString * __nonnull const CBObjCRuntimeErrorDomain;

/// `userInfo` key with additional information is interceptor probably KVO.
extern NSString * __nonnull const CBObjCRuntimeErrorIsKVOKey;

typedef NS_ENUM(NSInteger, CBObjCRuntimeError) {
    CBObjCRuntimeErrorUnknown                                           = 1,
    CBObjCRuntimeErrorObjectMessagesAlreadyBeingIntercepted             = 2,
    CBObjCRuntimeErrorSelectorNotImplemented                            = 3,
    CBObjCRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects = 4,
    CBObjCRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism  = 5,
    CBObjCRuntimeErrorSavingOriginalForwardingMethodFailed              = 6,
    CBObjCRuntimeErrorReplacingMethodWithForwardingImplementation       = 7,
    CBObjCRuntimeErrorObservingPerformanceSensitiveMessages             = 8,
    CBObjCRuntimeErrorObservingMessagesWithUnsupportedReturnType        = 9,
};

/// Transforms normal selector into a selector with CB prefix.
SEL _Nonnull CB_selector(SEL _Nonnull selector);
...
@protocol CBMessageSentObserver
...
@protocol CBDeallocatingObserver
...
IMP __nullable CB_ensure_observing(id __nonnull target, SEL __nonnull selector, NSError *__autoreleasing __nullable * __nullable error);

#endif

/// Extracts arguments for `invocation`.
NSArray * __nonnull CB_extract_arguments(NSInvocation * __nonnull invocation);
...
/// Default value for `CBInterceptionObserver.targetImplementation`.
