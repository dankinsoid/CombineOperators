...
#import "include/_CB.h"
#import "include/_CBObjCRuntime.h"
...
BOOL CBAbortOnThreadingHazard = NO;
...
typedef BOOL (^CBInterceptWithOptimizedObserver)(CBObjCRuntime * __nonnull self, Class __nonnull class, SEL __nonnull selector, NSErrorParam error);
...
#define CB_PREFIX @"_CB_namespace_"

#define CB_ARG_id(value)           ((value) ?: [NSNull null])
#define CB_ARG_char(value)         [NSNumber numberWithChar:value]
#define CB_ARG_short(value)        [NSNumber numberWithShort:value]
#define CB_ARG_int(value)          [NSNumber numberWithInt:value]
#define CB_ARG_long(value)         [NSNumber numberWithLong:value]
#define CB_ARG_BOOL(value)         [NSNumber numberWithBool:value]
#define CB_ARG_SEL(value)          [NSNumber valueWithPointer:value]
#define CB_ARG_rx_uchar(value)     [NSNumber numberWithUnsignedInt:value]
#define CB_ARG_rx_ushort(value)    [NSNumber numberWithUnsignedInt:value]
#define CB_ARG_rx_uint(value)      [NSNumber numberWithUnsignedInt:value]
#define CB_ARG_rx_ulong(value)     [NSNumber numberWithUnsignedLong:value]
#define CB_ARG_rx_block(value)     ((id)(value) ?: [NSNull null])
#define CB_ARG_float(value)        [NSNumber numberWithFloat:value]
#define CB_ARG_double(value)       [NSNumber numberWithDouble:value]
...
NSString * __nonnull const CBObjCRuntimeErrorDomain   = @"CBObjCRuntimeErrorDomain";
NSString * __nonnull const CBObjCRuntimeErrorIsKVOKey = @"CBObjCRuntimeErrorIsKVOKey";

BOOL CB_return_type_is_supported(const char *type) {
    if (type == nil) {
        return NO;
    }
...
static BOOL CB_method_has_supported_return_type(Method method) {
    ...
    return CB_return_type_is_supported(methodSignature.methodReturnType);
}

SEL __nonnull CB_selector(SEL __nonnull selector) {
    NSString *selectorString = NSStringFromSelector(selector);
    return NSSelectorFromString([CB_PREFIX stringByAppendingString:selectorString]);
}

#endif

BOOL CB_is_method_signature_void(NSMethodSignature * __nonnull methodSignature) {
    const char *methodReturnType = methodSignature.methodReturnType;
    return strcmp(methodReturnType, @encode(void)) == 0;
}

BOOL CB_is_method_with_description_void(struct objc_method_description method) {
    return strncmp(method.types, @encode(void), 1) == 0;
}

id __nonnull CB_extract_argument_at_index(NSInvocation * __nonnull invocation, NSUInteger index) {
...
NSArray *CB_extract_arguments(NSInvocation *invocation) {
    ...
    for (NSUInteger index = HIDDEN_ARGUMENT_COUNT; index < numberOfArguments; ++index) {
        [arguments addObject:CB_extract_argument_at_index(invocation, index) ?: [NSNull null]];
    }
    
    return arguments;
}

IMP __nonnull CB_default_target_implementation(void) {
    return _objc_msgForward;
}

#if !DISABLE_SWIZZLING

void * __nonnull CB_reference_from_selector(SEL __nonnull selector) {
    return selector;
}

static BOOL CB_forward_invocation(id __nonnull __unsafe_unretained self, NSInvocation *invocation) {
    SEL originalSelector = CB_selector(invocation.selector);

    id<CBMessageSentObserver> messageSentObserver = objc_getAssociatedObject(self, originalSelector);

    if (messageSentObserver != nil) {
        NSArray *arguments = CB_extract_arguments(invocation);
...
static BOOL CB_responds_to_selector(id __nonnull __unsafe_unretained self, SEL selector) {
...
static NSMethodSignatureRef CB_method_signature(id __nonnull __unsafe_unretained self, SEL selector) {
    Class class = object_getClass(self);
...
static NSString * __nonnull CB_method_encoding(Method __nonnull method) {
...
@interface CBObjCRuntime: NSObject
...
+(CBObjCRuntime*)instance;

-(void)performLocked:(void (^)(CBObjCRuntime* __nonnull))action;
...
+(void)registerOptimizedObserver:(CBInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector;

@end

/**
 All API methods perform work on locked instance of `CBObjCRuntime`. In that way it's easy to prove
 that every action is properly locked.
 */
IMP __nullable CB_ensure_observing(id __nonnull target, SEL __nonnull selector, NSErrorParam error) {
    ...
    @synchronized(target) {
        ...
        @synchronized([target class]) {
            [[CBObjCRuntime instance] performLocked:^(CBObjCRuntime * __nonnull self) {
...
#define FORWARD_BODY(invocation)                        if (CB_forward_invocation(self, NAME_CAT(_, 0, invocation))) { return; }

#define RESPONDS_TO_SELECTOR_BODY(selector)             if (CB_responds_to_selector(self, NAME_CAT(_, 0, selector))) return YES;

#define CLASS_BODY(...)                                 return actAsClass;

#define METHOD_SIGNATURE_FOR_SELECTOR_BODY(selector)                                            \
    NSMethodSignatureRef methodSignature = CB_method_signature(self, NAME_CAT(_, 0, selector)); \
    if (methodSignature != nil) {                                                               \
        return methodSignature;                                                                 \
    }

#define DEALLOCATING_BODY(...)                                                        \
    id<CBDeallocatingObserver> observer = objc_getAssociatedObject(self, rxSelector); \
    if (observer != nil && observer.targetImplementation == thisIMP) {                \
        [observer deallocating];                                                      \
    }

#define OBSERVE_BODY(...)                                                              \
    id<CBMessageSentObserver> observer = objc_getAssociatedObject(self, rxSelector);   \
                                                                                       \
    if (observer != nil && observer.targetImplementation == thisIMP) {                 \
...
#define BUILD_ARG_WRAPPER(type)                   CB_ARG_ ## type                                                     //CB_ARG_ ## type

#define CAT(_1, _2, head, tail)                   CB_CAT2(head, tail)
#define SEPARATE_BY_COMMA(_1, _2, head, tail)     head, tail
#define SEPARATE_BY_SPACE(_1, _2, head, tail)     head tail
#define SEPARATE_BY_UNDERSCORE(head, tail)        CB_CAT2(CB_CAT2(head, _), tail)

#define UNDERSCORE_TYPE_CAT(_1, index, type)      CB_CAT2(_, type)                                                    // generates -> _type
...
#define EXAMPLE_PARAMETER(_1, index, type)        CB_CAT2(_, type):(type)SEPARATE_BY_UNDERSCORE(type, index)          // generates -> _type:(type)type_0
#define SELECTOR_PART(_1, index, type)            CB_CAT2(_, type:)                                                   // generates -> _type:

#define COMMA_DELIMITED_ARGUMENTS(...)            CB_FOREACH(_, SEPARATE_BY_COMMA, NOT_NULL_ARGUMENT_CAT, ## __VA_ARGS__)
#define ARGUMENTS(...)                            CB_FOREACH_COMMA(_, NAME_CAT, ## __VA_ARGS__)
#define DECLARE_ARGUMENTS(...)                    CB_FOREACH_COMMA(_, TYPE_AND_NAME_CAT, ## __VA_ARGS__)

// optimized observe methods

#define GENERATE_SELECTOR_IDENTIFIER(...)         CB_CAT2(exampleSelector, CB_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))

#define GENERATE_METHOD_IDENTIFIER(...)           CB_CAT2(swizzle, CB_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))
...
#define BUILD_EXAMPLE_METHOD(return_value, ...) \
    +(return_value)CB_CAT2(CB_CAT2(example_, return_value), CB_FOREACH(_, SEPARATE_BY_SPACE, EXAMPLE_PARAMETER, ## __VA_ARGS__)) {}

#define BUILD_EXAMPLE_METHOD_SELECTOR(return_value, ...) \
    CB_CAT2(CB_CAT2(example_, return_value), CB_FOREACH(_, SEPARATE_BY_SPACE, SELECTOR_PART, ## __VA_ARGS__))
...
#define SWIZZLE_OBSERVE_METHOD_BODY(return_value, ...)                                                                                                       \
       ...
       [self registerOptimizedObserver:^BOOL(CBObjCRuntime * __nonnull self, Class __nonnull class,                                                     \
...
method_prototype                                                                                                         \
    __unused SEL rxSelector = CB_selector(selector);                                                                     \
...
@interface CBObjCRuntime (InfrastructureMethods)
@end

// MARK: Infrastructure Methods

@implementation CBObjCRuntime (InfrastructureMethods)
...
@interface CBObjCRuntime (swizzle)

@end

@implementation CBObjCRuntime(swizzle)
...
@implementation CBObjCRuntime

static CBObjCRuntime *_instance = nil;
static NSMutableDictionary<NSString *, CBInterceptWithOptimizedObserver> *optimizedObserversByMethodEncoding = nil;
...
+(void)initialize {
    _instance = [[CBObjCRuntime alloc] init];
    defaultTypeID = CFGetTypeID((CFTypeRef)CBObjCRuntime.class); // just need a reference of some object not from CF
...
+(void)registerOptimizedObserver:(CBInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector {
    ...
    NSString *methodEncoding = CB_method_encoding(exampleEncodingMethod);
...
-(IMP __nullable)ensurePrepared:(id __nonnull)target forObserving:(SEL __nonnull)selector error:(NSErrorParam)error {
    ...
    if (instanceMethod == nil) {
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorSelectorNotImplemented
                                       userInfo:nil], nil);
    }
    ...
    ||  selector == @selector(respondsToSelector:)) {
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorObservingPerformanceSensitiveMessages
                                       userInfo:nil], nil);
    }
    ...
    if (selector == deallocSelector) {
        ...
        NSString *methodEncoding = CB_method_encoding(instanceMethod);
        CBInterceptWithOptimizedObserver optimizedIntercept = optimizedObserversByMethodEncoding[methodEncoding];

        if (!CB_method_has_supported_return_type(instanceMethod)) {
            CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                               code:CBObjCRuntimeErrorObservingMessagesWithUnsupportedReturnType
                                           userInfo:nil], nil);
        }
        ...
        // default fallback to observing by forwarding messages
            ...
            if ([self forwardingSelector:selector forClass:swizzlingImplementorClass]) {
                return CB_default_target_implementation();
            }
    ...
    CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                       code:CBObjCRuntimeErrorUnknown
                                   userInfo:nil], nil);
}
...
-(Class __nullable)prepareTargetClassForObserving:(id __nonnull)target error:(NSErrorParam)error {
    ...
    if (isThisTollFreeFoundationClass) {
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects
                                       userInfo:nil], nil);
    }
     ...
     * `sentMessage` sets object class to be `_CB_namespace___KVO__OriginalClass` (subclass of `__KVO__OriginalClass`)
    ...
    if ([target class] != object_getClass(target)) {
        ...
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorObjectMessagesAlreadyBeingIntercepted
                                       userInfo:@{
                                                  CBObjCRuntimeErrorIsKVOKey : @(isKVO)
                                                  }], nil);
    }
    ...
    if (previousClass != wannaBeClass) {
        ...
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism
                                       userInfo:nil], nil);
    }
...
-(BOOL)observeByForwardingMessages:(Class __nonnull)swizzlingImplementorClass
...
#endif
    SEL rxSelector = CB_selector(selector);
    ...
    if (!class_addMethod(swizzlingImplementorClass, rxSelector, implementation, methodEncoding)) {
        CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                           code:CBObjCRuntimeErrorSavingOriginalForwardingMethodFailed
                                       userInfo:nil], NO);
    }
    ...
    if (!class_addMethod(swizzlingImplementorClass, selector, _objc_msgForward, methodEncoding)) {
        if (implementation != method_setImplementation(instanceMethod, _objc_msgForward)) {
            ...
            CB_THROW_ERROR([NSError errorWithDomain:CBObjCRuntimeErrorDomain
                                               code:CBObjCRuntimeErrorReplacingMethodWithForwardingImplementation
                                           userInfo:nil], NO);
        }
    }
...
-(Class __nullable)ensureHasDynamicFakeSubclass:(Class __nonnull)class error:(NSErrorParam)error {
    ...
    NSString *dynamicFakeSubclassName = [CB_PREFIX stringByAppendingString:NSStringFromClass(class)];
...
NSInteger CB_number_of_dynamic_subclasses(void) {
...
NSInteger CB_number_of_forwarding_enabled_classes(void) {
    __block NSInteger count = 0;
    [[CBObjCRuntime instance] performLocked:^(CBObjCRuntime * __nonnull self) {
...
NSInteger CB_number_of_intercepting_classes(void) {
    __block NSInteger count = 0;
    [[CBObjCRuntime instance] performLocked:^(CBObjCRuntime * __nonnull self) {
...
NSInteger CB_number_of_forwarded_methods(void) {
    return numberOfForwardedMethods;
}

NSInteger CB_number_of_swizzled_methods(void) {
