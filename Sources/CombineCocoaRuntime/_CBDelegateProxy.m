...
#import "include/_CBDelegateProxy.h"
#import "include/_CB.h"
#import "include/_CBObjCRuntime.h"

@interface _CBDelegateProxy () {
...
@implementation _CBDelegateProxy
...
+(NSSet*)collectVoidSelectorsForProtocol:(Protocol *)protocol {
    ...
    for (unsigned int i = 0; i < protocolMethodCount; ++i) {
        ...
        if (CB_is_method_with_description_void(method)) {
...
+(void)initialize {
    @synchronized (_CBDelegateProxy.class) {
...
-(void)forwardInvocation:(NSInvocation *)anInvocation {
    BOOL isVoid = CB_is_method_signature_void(anInvocation.methodSignature);
    NSArray *arguments = nil;
    if (isVoid) {
        arguments = CB_extract_arguments(anInvocation);
